...
from apps.imports.services.fuel import reapply_active_fuel_rates
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["import_workbook"]:
            ...
            if client:
                restored = reapply_active_fuel_rates(client)
                if restored.get("active_fuel_file_id"):
                    self.stdout.write(
                        self.style.WARNING(
                            f"Restored active Admin fuel dataset #{restored['active_fuel_file_id']} "
                            f"to {restored['configs_reapplied']} carrier configs after historical validation."
                        )
                    )
    ...
    def _import_workbook(self, workbook_path: Path, client_code: str, replace: bool) -> None:
        ...
        args.extend(["--fuel-source", "workbook"])
