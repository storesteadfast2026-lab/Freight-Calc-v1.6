...
## LOGIN-FINDING-0724.1358 - User enumeration difference removed

Initial testing found that valid credentials without a calculator profile returned `This user does not have a calculator access profile`, while unknown users returned a different message. This exposed account state. The login boundary now returns one generic message and includes regression tests for unknown user, wrong password, missing profile, disabled access and valid Customer User.
