...
The memory review queue also exposes transversal groups for exact previous
solutions, changed Source values, new issues without memory, and rows matching
an active reusable rule. **Reuse all previous approved solutions** prepares or
refreshes drafts only for exact Source fingerprints; applied decisions and
changed Source values are not silently selected.

**Download memory Excel** exports the client's Product reconciliation memories
as a flattened `.xlsx` review report. The general correction-memory Admin
download respects its current filters and search. Exports contain Source,
approved values, field decisions, provenance, reason, user, timestamps and
reuse count. The writer streams database rows and spools output to disk. Above
25,000 records it returns a ZIP with numbered Excel parts, making the result
practical for human or AI-assisted review. The modern `.xlsx` format is used
because legacy `.xls` is limited to 65,536 rows. Export is read-only and is
audited.
