...
def _change_kind(row, previous_values):
    """Classify which routing attributes differ at an existing location."""
    same_zone = [(z, sz, a) for z, sz, a in previous_values if z == row['zone']]
    if not same_zone:
        return 'ZONE'
    if any(sz == row['subzone'] for _, sz, _ in same_zone):
        return 'AREA_ONLY'
    if any(a == row['area'] for _, _, a in same_zone):
        return 'SUBZONE_ONLY'
    return 'SUBZONE_AND_AREA'
...
def validate_zones_file(external_file: ExternalDataFile, actor=None) -> dict:
    ...
    try:
        ...
        current_by_carrier = Counter()
        carrier_by_service_id = {}
        for service_id, carrier, service, suburb, state, postcode, zone, subzone, area in existing_rows:
            carrier = _u(carrier)
            carrier_by_service_id[service_id] = carrier
            key = (
        ...
        source_locations_by_service = defaultdict(set)
        change_samples = defaultdict(list)
        ...
        for row in parsed_rows:
            ...
            for service in applicable_services:
                ...
                source_locations_by_service[service_id].add(
                ...
                if decision == 'CANDIDATE_ADD':
                    ...
                    change_kind = _change_kind(row, previous_values)
                    counts['candidate_change'] += 1
                    counts[f"change_{change_kind.lower()}"] += 1
                    per_carrier[carrier]['candidate_change'] += 1
                    per_carrier[carrier][f"change_{change_kind.lower()}"] += 1
                    sample_key = (carrier, change_kind)
                    if len(change_samples[sample_key]) < 3:
                        change_samples[sample_key].append({
                            'carrier': carrier,
        ...
        current_detail_diff = 0
        current_location_absent = 0
        current_detail_by_carrier = Counter()
        current_absent_by_carrier = Counter()
        for service_id, current_rows in existing_relevant_by_service.items():
            source_rows = source_full_by_service.get(service_id, set())
            source_locations = source_locations_by_service.get(service_id, set())
            carrier = carrier_by_service_id.get(service_id, '-')
            for current_key in current_rows:
                if current_key in source_rows:
                    continue
                _service_id, suburb, state, postcode, _zone, _subzone, _area = current_key
                if (suburb, state, postcode) in source_locations:
                    current_detail_diff += 1
                    current_detail_by_carrier[carrier] += 1
                else:
                    current_location_absent += 1
                    current_absent_by_carrier[carrier] += 1
        current_not_in_source = current_detail_diff + current_location_absent
        ...
        if current_not_in_source:
            warnings.append(
                f'{current_not_in_source} current relevant Django FreightZone row(s) are not exact source matches: '
                f'{current_detail_diff} are at source-present locations but differ in zone/subzone/area, and '
                f'{current_location_absent} are at locations absent from the safely mapped source. '
                'All remain PRESERVE EXISTING in this phase.'
            )
        ...
        summary = {
            ...
            'change_zone': counts['change_zone'],
            'change_subzone_only': counts['change_subzone_only'],
            'change_area_only': counts['change_area_only'],
            'change_subzone_and_area': counts['change_subzone_and_area'],
            'current_relevant_not_in_source': current_not_in_source,
            'current_detail_diff': current_detail_diff,
            'current_location_absent': current_location_absent,
            ...
            'carrier_summary': [
                {
                    ...
                    'change_zone': values['change_zone'],
                    'change_subzone_only': values['change_subzone_only'],
                    'change_area_only': values['change_area_only'],
                    'change_subzone_and_area': values['change_subzone_and_area'],
                    'current_rows': current_by_carrier[carrier],
                    'current_detail_diff': current_detail_by_carrier[carrier],
                    'current_location_absent': current_absent_by_carrier[carrier],
                }
                for carrier, values in sorted(per_carrier.items())
            ],
            'change_samples': [
                sample
                for key in sorted(change_samples)
                for sample in change_samples[key]
            ],
        ...
        create_audit_event(
            ...
            metadata={
                ...
                'change_zone': summary['change_zone'],
                'change_subzone_only': summary['change_subzone_only'],
                'change_area_only': summary['change_area_only'],
                'change_subzone_and_area': summary['change_subzone_and_area'],
                'current_detail_diff': summary['current_detail_diff'],
                'current_location_absent': summary['current_location_absent'],
                'database_updated': False,
            },
        )
        return summary
