...
## FTP Zones Phase 1 - 2026-08-27

Zones follows the same controlled external-data pattern used for Fuel and Postcodes:

```text
uploaded_data/zones.csv
  -> immutable snapshot + SHA-256
  -> schema/index validation
  -> exact-duplicate detection
  -> Australian postcode/state review
  -> carrier-to-service mapping review
  -> Suburb and FreightRate cross-check
  -> current-vs-source delta preview
  -> STOP (no activation in Phase 1)
```

Important constraint: the FTP Zones file identifies `carrier` but not Django `CarrierService`. A source carrier may be mapped only when the current STH configuration provides one safe service interpretation. Ambiguous mappings remain review-only.

Missing current Django zones are `PRESERVE EXISTING` during this phase. No replace/delete semantics are permitted.

## Zones service derivation rule - 2026-08-27

`zones.csv` is carrier-level routing data and intentionally omits service. Django may materialise the same source destination/zone under one or more CarrierService records. Applicable services are determined dynamically from active client carrier/service configuration plus FreightRate availability for the source zone.

Do not hard-code ROAD, GENERAL, PRIORITY, SATCHEL or any future service names. A carrier with multiple configured services is valid. A source zone is applicable to every configured service that currently has a FreightRate for that zone.

This rule supersedes the earlier Phase 1 heuristic that selected a single service from existing FreightZone data.

## Zones routing-difference diagnostics - 2026-08-27

`zones.csv` remains carrier-level and service applicability remains rate-driven. Validation version 3 adds a diagnostic layer before activation design. It distinguishes a real zone change from changes limited to `subzone` and/or `area`, and it separates current rows at source-present destinations from current destinations absent from the safely mapped source.

These diagnostics are review evidence only; they do not authorise automatic replacement.
