...
Validation feedback is field-specific. A missing common review note is shown
once beside the common note field and does not make every selected row appear
invalid. A genuinely invalid row is opened automatically, highlighted, and
each invalid field shows its own message. The summary links directly to the
affected field.

### 3.2 Reusable correction memory

The common bulk-review framework stores approved corrections in
`ExternalDataCorrectionMemory`. A memory entry is scoped by workflow and
client and contains:

- a SHA-256 fingerprint of the original source values and column count;
- the record key (Product SKU for the first adapter);
- the exact approved reconstruction and its fingerprint;
- the approving user, timestamp, note, origin file and reuse count.

When a later upload contains exactly the same malformed source values, the
previously approved reconstruction is loaded into the new pending row and the
screen shows `Previously approved — identical source`. The user must still
select the row, provide a note and approve it for the new source file. Nothing
is accepted silently.

If the same record key is found but any original source value has changed, the
old reconstruction is not loaded. The screen shows
`Previous correction found — source changed`, and the current proposal remains
subject to manual review. A new source with no match shows
`No previous correction`.

Migration `imports.0011` backfills the memory from repairs already approved
before this feature was installed. The service and database model are generic;
future file adapters may reuse them by supplying their own workflow key,
record key, source snapshot and approved data.
