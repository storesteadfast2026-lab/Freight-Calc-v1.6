...
zones or freight type. The C/P value is protected and is never inferred from
the source `pallet` column. The source rule remains informational only:
`pallet = 0` means Case and `pallet > 0` means Pallet.
...
validated file exposes a read-only **Product reconciliation** operation. The
...
- source-only SKU (new candidates requiring an explicit C/P decision);
...
Source millimetres are converted to metres for comparison only. The screen has
no Apply, Approve or update action. It does not modify `Product`, Calculator,
rates, C/P, or either source table. If any rejected row is still pending, the
comparison is visibly blocked until rejected-row review is complete.

The reconciliation page is server-side paginated (100 records per page),
searchable by SKU/name and filterable by state. This is the diagnostic step
before a separate, future controlled approval procedure is designed.
