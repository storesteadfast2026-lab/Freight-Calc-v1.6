...
RECONCILABLE_FIELDS = ('name', 'description', 'dimensions', 'weight', 'cubic')
...
def build_workspace(external_file):
    ...
    summary['pending_decisions'] = max(
        0,
        summary['different'] + summary['operational_only'] - summary['draft_decisions'],
    )
    return rows, summary
...
def preview_decision(row, decision):
    ...
    proposed = {
        ...
        # C/P remains operational and cannot be set by a reconciliation draft.
        'freight_type': operational.get('freight_type'),
    }
    return {
        'row': row,
        'decision': decision,
