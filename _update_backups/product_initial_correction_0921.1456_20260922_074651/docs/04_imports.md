...
no operational Apply action. It does not modify `Product`, Calculator, rates,
C/P, or either source table. Its C/P proposal uses `pallet = 0 → C` and
`pallet > 0 → P`; invalid values are routed to manual review. If any rejected row is still pending, the
