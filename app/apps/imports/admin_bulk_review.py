...
from apps.imports.services.correction_memory import (
    find_correction_memory,
    remember_correction,
)
from apps.imports.services.xlsx_reader import normalize_product_sku, value_to_text
...
class BulkReviewWorkflow(ABC):
    ...
    @staticmethod
    def _error(message, *, row_id='', field_id=''):
        return {
            'message': message,
            'row_id': row_id,
            'field_id': field_id,
    ...
    def _form_errors(self, item, form):
        details = []
        for field_name, errors in form.errors.items():
            if field_name == '__all__':
                label = 'Row'
                target = f'bulk-review-row-{item.pk}'
            else:
                label = form.fields[field_name].label or field_name.replace('_', ' ').title()
                target = form[field_name].id_for_label
            for error in errors:
                details.append(self._error(
                    f'Row {self.item_label(item)} — {label}: {error}',
                    row_id=str(item.pk),
                    field_id=target,
                ))
        return details
    ...
    def __call__(self, request, object_id):
        ...
        if request.method == 'POST':
            ...
            if not approve and not save:
                page_errors.append(self._error('Choose Save selected proposals or Approve selected into staging.'))
            if not selected_items:
                page_errors.append(self._error('Select at least one pending row.'))
            if approve and not note:
                page_errors.append(self._error(
                    'Enter a review note before approving selected rows.',
                    field_id='bulk-review-note',
                ))
            if len(selected_items) != len(selected_ids):
                page_errors.append(self._error('One or more selected rows are no longer available for review.'))

            for item in selected_items:
                form = self._bound_form(request, item, approve=approve, note=note)
                bound_forms[item.pk] = form
                if not form.is_valid():
                    page_errors.extend(self._form_errors(item, form))
            ...
            if not page_errors:
                try:
                    ...
                    page_errors.append(self._error(str(exc)))
                else:
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            'note_has_error': any(
                error.get('field_id') == 'bulk-review-note' for error in page_errors
            ),
...
class ProductRejectedRowsBulkReview(BulkReviewWorkflow):
    ...
    def __init__(self, admin_site):
        ...
        self._memory_matches = {}

    @staticmethod
    def _memory_source_data(item):
        return {
            'column_count': item.column_count,
            'raw_values': item.raw_values or [],
        }

    def _record_key(self, item):
        proposed_code = (item.proposed_data or {}).get('code')
        raw_code = item.raw_values[0] if item.raw_values else ''
        return normalize_product_sku(proposed_code or raw_code)

    def _memory_match(self, item):
        if item.pk not in self._memory_matches:
            self._memory_matches[item.pk] = find_correction_memory(
                workflow_key='product_rejected_rows',
                client=item.external_file.client,
                record_key=self._record_key(item),
                source_data=self._memory_source_data(item),
            )
        return self._memory_matches[item.pk]
    ...
    def build_form(self, item, *, data=None):
        memory_match = self._memory_match(item)
        form = ProductSourceRejectedRowReviewForm(
            data=data,
            instance=item,
            prefix=f'row-{item.pk}',
        )
        # A newly validated row already contains the automatic reconstruction in
        # proposed_data. Exact memory must replace that draft, while a proposal
        # explicitly saved by a reviewer remains authoritative for this upload.
        if data is None and item.repair_status == 'PENDING' and memory_match.can_reuse:
            for field_name, value in memory_match.memory.approved_data.items():
    ...
    def build_row(self, item, form) -> dict:
        ...
        memory_match = self._memory_match(item)
        memory = memory_match.memory
        if memory_match.state == 'EXACT':
            memory_label = 'Previously approved — identical source'
            memory_css = 'bulk-review-memory-match'
        elif memory_match.state == 'SOURCE_CHANGED':
            memory_label = 'Previous correction found — source changed'
            memory_css = 'bulk-review-memory-changed'
        else:
            memory_label = 'No previous correction'
            memory_css = 'bulk-review-memory-new'
        return {
            'id': item.pk,
            ...
            'memory_state': memory_match.state,
            'memory_label': memory_label,
            'memory_css': memory_css,
            'memory_approved_by': (
                memory.approved_by.get_username() if memory and memory.approved_by else ''
            ),
            'memory_approved_at': memory.approved_at if memory else None,
            'memory_use_count': memory.use_count if memory else 0,
            'display_cells': [
    ...
    def process_item(self, request, item, form, *, approve: bool, note: str):
        ...
        if approve:
            remember_correction(
                workflow_key='product_rejected_rows',
                client=item.external_file.client,
                record_key=normalize_product_sku(payload.get('code')),
                source_data=self._memory_source_data(item),
                approved_data=updated.proposed_data,
                approval_note=note,
                origin_external_file=item.external_file,
                actor=request.user,
                request=request,
            )
        return updated
