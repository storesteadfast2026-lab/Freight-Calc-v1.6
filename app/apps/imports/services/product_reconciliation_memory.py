...
def _require_editable_source(external_file):
    from apps.imports.services.product_reconciliation_workspace import has_active_product_apply
    ...
    if has_active_product_apply(external_file):
        raise ProductReconciliationMemoryError(
            'This Product source has an active Apply. Roll it back before preparing decisions.'
        )
...
def prepare_exact_memory_drafts(external_file, *, actor=None, request=None):
    ...
    _require_editable_source(external_file)
...
@transaction.atomic
def reuse_all_exact_product_memories(external_file, *, actor=None, request=None):
    """Explicit bulk refresh of every compatible exact previous solution."""
    from apps.imports.services.product_reconciliation_workspace import build_workspace
    ...
    for row in rows:
        ...
        if (
            row.get('memory_state') != 'EXACT'
            or memory is None
            or memory.origin_external_file_id == external_file.pk
            or row['status'] == 'SAME'
        ):
            continue
        if decision is not None and decision.decision_status == 'APPLIED':
            skipped_applied.append(row['sku'])
            continue
        saved = _save_memory_draft(external_file, row, memory, actor)
    ...
    create_audit_event(
        ...
        message=(
            f'{len(prepared)} exact previous Product solution(s) prepared or refreshed '
            'as drafts.'
        ),
        actor=actor,
        client=external_file.client,
        ...
        metadata={
            'decision_count': len(prepared),
            'memory_ids': used_memory_ids[:500],
            'skipped_applied_skus': skipped_applied[:500],
            'exact_source_match_required': True,
            'operational_tables_updated': False,
        },
        request=request,
