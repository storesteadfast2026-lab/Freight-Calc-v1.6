...
def create_recommended_product_drafts(external_file, *, actor=None, request=None):
    ...
    if has_active_product_apply(external_file):
        raise ProductReconciliationApplyBlocked(
            'This Product source has an active Apply. Roll it back before preparing corrections.'
        )
...
def apply_product_reconciliation(external_file_id, *, actor=None, request=None):
    ...
    if has_active_product_apply(external_file):
        raise ProductReconciliationApplyBlocked(
            'This Product source already has an active Apply. Roll it back before another Apply.'
        )
...
def rollback_latest_product_apply(external_file_id, *, actor=None, request=None):
    external_file = ExternalDataFile.objects.select_for_update().get(pk=external_file_id)
    if external_file.product_correction_rounds.filter(status='APPLIED').exists():
        raise ProductReconciliationApplyBlocked(
            'Roll back later correction rounds before rolling back the initial Apply.'
        )
