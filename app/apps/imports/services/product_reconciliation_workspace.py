...
def has_active_product_apply(external_file):
    """An active Apply makes this source read-only until it is rolled back."""
    return any(
        batch.get('batch_id') and not batch.get('rolled_back_at')
        for batch in (external_file.import_summary or {}).get(
            'product_reconciliation_apply_batches', []
        )
...
def reconciliation_group_counts(rows):
    ...
    for row in rows:
        ...
        elif row['status'] not in {'SAME', 'SOURCE_ONLY'}:
...
def rows_for_group(rows, group_key):
    ...
    if group_key == 'MEMORY_NEW_ISSUE':
        return [
            row for row in rows
            if row['status'] not in {'SAME', 'SOURCE_ONLY'}
            and row.get('memory_state') == 'NONE'
        ]
...
def build_workspace(external_file):
    ...
    decision_counts = Counter(
        row['decision'].decision_status for row in rows if row.get('decision')
    )
    summary['draft_decisions'] = decision_counts['DRAFT']
    summary['ready_decisions'] = decision_counts['READY']
    summary['applied_decisions'] = decision_counts['APPLIED']
    summary['pending_drafts'] = (
        summary['draft_decisions'] + summary['ready_decisions']
    )
    summary['reusable_previous_solutions'] = sum(
        row.get('memory_state') == 'EXACT'
        and row.get('memory') is not None
        and row['memory'].origin_external_file_id != external_file.pk
        and row['status'] != 'SAME'
        and (
            row.get('decision') is None
            or row['decision'].decision_status != 'APPLIED'
        )
        for row in rows
    )
...
def save_product_reconciliation_decisions(
    ...
    *,
    ...
    if has_active_product_apply(external_file):
        raise ProductReconciliationWorkspaceError(
            'This Product source has an active Apply. Roll it back before editing decisions.'
        )
