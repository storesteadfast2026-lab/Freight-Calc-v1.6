...
class ProductReconciliationWorkspaceTests(TestCase):
    ...
    def test_two_correction_rounds_preserve_initial_decision_and_rollback_in_order(self):
        create_recommended_product_drafts(self.external_file, actor=self.user)
        apply_product_reconciliation(self.external_file.pk, actor=self.user)
        initial = ProductReconciliationDecision.objects.get(external_file=self.external_file)
        self.assertEqual(initial.decision_status, 'APPLIED')
        authorities = {
            'name': 'NO_CHANGE', 'description': 'NO_CHANGE',
            'dimensions': 'OPERATIONAL', 'weight': 'SOURCE',
            'cubic': 'OPERATIONAL', 'freight_type': 'OPERATIONAL',
        }
        first = start_correction_round(self.external_file.pk, actor=self.user)
        save_correction_decision(
            first.pk, sku='20772', field_decisions=authorities,
            notes='Verified the Source weight against the physical item.', actor=self.user,
        )
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('70'))
        self.assertTrue(correction_preview(first)['can_apply'])
        apply_correction_round(first.pk, actor=self.user)
        first.refresh_from_db()
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('45.2'))
        memory = ExternalDataCorrectionMemory.objects.get(
            client=self.client_obj, record_key='20772', is_active=True
        )
        self.assertEqual(memory.approved_data['apply_batch_id'], first.apply_batch_id)
        self.assertEqual(initial.pk, ProductReconciliationDecision.objects.get(external_file=self.external_file).pk)
        self.assertEqual(ProductReconciliationDecision.objects.get(pk=initial.pk).decision_status, 'APPLIED')
        with self.assertRaises(ProductReconciliationApplyBlocked):
            rollback_latest_product_apply(self.external_file.pk, actor=self.user)

        second = start_correction_round(self.external_file.pk, actor=self.user)
        authorities['weight'] = 'CUSTOM'
        save_correction_decision(
            second.pk, sku='20772', field_decisions=authorities,
            custom_values={'weight_kg': '50'}, notes='New verified weight.', actor=self.user,
        )
        apply_correction_round(second.pk, actor=self.user)
        second.refresh_from_db()
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('50'))
        memory.refresh_from_db()
        self.assertEqual(memory.approved_data['apply_batch_id'], second.apply_batch_id)
        with self.assertRaises(CorrectionRoundError):
            rollback_correction_round(first.pk, actor=self.user)
        rollback_correction_round(second.pk, actor=self.user)
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('45.2'))
        memory.refresh_from_db()
        self.assertEqual(memory.approved_data['apply_batch_id'], first.apply_batch_id)
        rollback_correction_round(first.pk, actor=self.user)
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('70'))
        self.assertEqual(ProductCorrectionRound.objects.filter(external_file=self.external_file).count(), 2)

    def test_correction_preview_blocks_product_drift(self):
        create_recommended_product_drafts(self.external_file, actor=self.user)
        apply_product_reconciliation(self.external_file.pk, actor=self.user)
        round_obj = start_correction_round(self.external_file.pk, actor=self.user)
        save_correction_decision(
            round_obj.pk, sku='20772',
            field_decisions={'name': 'NO_CHANGE', 'description': 'NO_CHANGE',
                             'dimensions': 'OPERATIONAL', 'weight': 'SOURCE',
                             'cubic': 'OPERATIONAL', 'freight_type': 'OPERATIONAL'},
            notes='Verified Source weight.', actor=self.user,
        )
        self.product.refresh_from_db()
        self.product.weight_kg = Decimal('72')
    ...
    def test_correction_round_is_accessible_from_applied_workspace(self):
        ...
        response = self.client.get(url)
        self.assertContains(response, 'Review protected differences / correction rounds')
        response = self.client.post(f'{url}?mode=correction', {'correction_action': 'start'})
        self.assertEqual(response.status_code, 302)
        round_obj = ProductCorrectionRound.objects.get(external_file=self.external_file)
        response = self.client.get(f'{url}?mode=correction&round={round_obj.pk}&edit=20772')
        self.assertContains(response, 'Review 20772')
        self.assertContains(response, 'Save correction draft')
        response = self.client.post(f'{url}?mode=correction', {
            'correction_action': 'save', 'round_id': round_obj.pk, 'sku': '20772',
            'name_authority': 'NO_CHANGE', 'description_authority': 'NO_CHANGE',
            'dimensions_authority': 'OPERATIONAL', 'weight_authority': 'SOURCE',
            'cubic_authority': 'OPERATIONAL', 'freight_type_authority': 'OPERATIONAL',
            'notes': 'Verified weight with warehouse team.',
        })
        self.assertEqual(response.status_code, 302)
        self.assertEqual(round_obj.decisions.count(), 1)
        self.assertEqual(Product.objects.get(pk=self.product.pk).weight_kg, Decimal('70'))
        response = self.client.get(f'{url}?mode=correction&round={round_obj.pk}&preview=1')
